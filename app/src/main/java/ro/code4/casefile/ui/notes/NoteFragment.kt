...
import ro.code4.casefile.adapters.NoteDelegationAdapter
import ro.code4.casefile.helper.Constants
import ro.code4.casefile.helper.Constants.REQUEST_CODE_GALLERY
import ro.code4.casefile.helper.Constants.REQUEST_CODE_RECORD_VIDEO
import ro.code4.casefile.helper.Constants.REQUEST_CODE_TAKE_PHOTO
import ro.code4.casefile.helper.PermissionManager
import ro.code4.casefile.helper.TextWatcherDelegate
import ro.code4.casefile.helper.hideKeyboard
import ro.code4.casefile.helper.openGallery
import ro.code4.casefile.helper.takePicture
import ro.code4.casefile.helper.takeVideo
...
class NoteFragment : ViewModelFragment<NoteViewModel>(), PermissionManager.PermissionListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val questionId = Parcels.unwrap<Int>(arguments?.getParcelable((Constants.QUESTION)))
        val formId = Parcels.unwrap<Int>(arguments?.getParcelable((Constants.FORM)))
        val beneficiaryId = Parcels.unwrap<Int>(arguments?.getParcelable((Constants.PATIENT)))
        val editNoteId = Parcels.unwrap<Int>(arguments?.getParcelable((Constants.EDIT_NOTE)))

        viewModel.setData(questionId, formId, beneficiaryId, editNoteId)
        ...
        if (editNoteId != null) {
            viewModel.editNote().observe(viewLifecycleOwner, Observer { uiModel ->
                noteInput.setText(uiModel.name)
            })
        }
    ...
    private fun setupNotesList() {
        ...
        notesList.addItemDecoration(
            HorizontalDividerItemDecoration.Builder(mContext)
                ...
                .sizeResId(R.dimen.margin_8dp).build()
        )
    }
