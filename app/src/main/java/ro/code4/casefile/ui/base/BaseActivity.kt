...
import ro.code4.casefile.helper.APIError400
import ro.code4.casefile.helper.LocaleManager
import ro.code4.casefile.helper.fromJson
import ro.code4.casefile.helper.lifecycle.ActivityCallbacks
    ...
    fun showPushNotification(
        ...
        if (!isFinishing && dialog == null) {
            dialog = AlertDialog.Builder(this, R.style.AlertDialog)
                .setTitle(title)
                .setMessage(body)
                .setPositiveButton(R.string.push_notification_ok) { p0, _ ->
                    okListener?.invoke()
                    p0.dismiss()
                }
                .setCancelable(false)
            ...
            is HttpException -> {
                if (exception.code() == 400) {
                    ...
                    apiError400?.let {
                        ...
                        val valueInPixels = resources.getDimension(R.dimen.margin_24dp).toInt()
                        ...
                        builder.setTitle(getString(R.string.error_generic))
                            .setCancelable(false)
                            .setPositiveButton(R.string.push_notification_ok) { p0, _ -> p0.dismiss() }
                            .setCancelable(false)
